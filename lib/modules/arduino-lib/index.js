...
function Library (options) {
    ...
    var _init = function() {
        ...
        internals.serialPort.open(function (err) {
            ...
            // listen for the current speed once
            self.events.once('servo_pos', function (data) {

                // success, connection is open
                self.events.emit('open');
            });

            // get the current position (speed)
            setTimeout(function () {

                internals.getServoSpeed();
            }, 3000);
        });
    };
